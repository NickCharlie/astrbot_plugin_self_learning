...
- 修复审查反馈：解析失败的行保留在导入报告的失败明细中并计入总数；全部无效时返回逐行明细而非笼统报错。
- 修复审查反馈：`is_global` 严格解析（布尔与常见字符串形式，其余一律 False），宽松调用方传 `"false"` 不再误开全局；蓝图与插件页桥接原样透传由服务层统一解析。
