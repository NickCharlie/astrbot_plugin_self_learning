...
- 新增独立 MCP Server，可在 AstrBot 外通过 `stdio`、`sse`、`shttp`/`streamable-http` 访问自学习数据。
- MCP 工具覆盖健康检查、配置脱敏、统计、群组消息、黑话查询/审查、表达审查和人格审查等常用读写路径。
- 新增 MCP 运行文档与 `requirements-mcp.txt`，并延迟数据库工厂导入，避免 MCP 场景过早加载 AstrBot 运行时事件类型。
