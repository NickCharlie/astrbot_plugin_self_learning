...
影子模式从插件已学习到的群聊记录中选取一位群友，分析其语言行为（句长、语气结尾、标点、问句/感叹比例、代表性表达），生成一份语言行为档案。机器人回复时会把这份档案注入 LLM 请求，使回复模仿该用户的语言节奏和表达习惯。

行为边界:

- 被动生效: 只影响机器人本来就发出的回复，不会让机器人主动发言，也不会产生额外回复。
- 只模仿语言风格，不冒充身份: 提示词明确要求不声称拥有该用户的身份、经历或观点。
- 对群内完全静默: 机器人不会提示影子模式或档案的存在，因此效果只能从回复风格变化上观察。

使用前提:

1. 已配置数据库，且插件已采集到目标群的消息（`live` 来源），或已通过功能融合页导入 QQ 聊天记录（`imported` 来源）。
2. 已开启 LLM Hook 总开关: 设置页 → `运行与内部` → `启用 LLM Hook 上下文注入`（`enable_llm_hooks`，默认关闭）。社交上下文、记忆、黑话、few-shot 和影子模式档案都经由该开关注入，开关关闭时不会注入任何内容。

操作步骤:

1. 打开 Dashboard `影子模式` 页。
2. 选择聊天记录来源: `live`（插件实时采集）或 `imported`(导入的历史记录)。
3. 选择来源群组，候选列表会显示每位群友的消息数；至少 3 条有效文本才会标记为可学习。
4. 填写生效群组: 档案在哪个群生效，通常与来源群组相同。
5. 选择群友并点击 `学习并启用`。学习会分析该用户最近最多 500 条消息中的有效文本（长度不小于 2、非纯媒体占位、不含指令性内容），同一来源重复学习会覆盖并更新档案。
6. 在生效群组正常与机器人对话，观察回复的句长、语气词、标点和表达习惯变化。

规则与限制:

- 同一生效群组同时只启用一份档案，启用新档案会自动停用该群组其它档案。
- 上下文检索与其它 LLM Hook 上下文并行执行，单源超时由 `llm_hook_context_timeout`（默认 3 秒）控制。
- 档案包含统计数据和最多 10 条代表性表达，注入位置为用户消息尾部的 `<context>` 块（`extra_user_content_parts`），不修改 system prompt，不影响 provider 前缀缓存。

启用了档案但群里看不到变化时，按 [常见问题](#影子模式启用了但没有效果) 排查。
...
影子模式只在 LLM Hook 注入开启时生效，且效果体现在回复风格上，本身较为克制。按顺序检查:

1. 设置页 → `运行与内部` → `启用 LLM Hook 上下文注入`（`enable_llm_hooks`）是否开启。该开关默认关闭，关闭时影子档案不会注入，这是最常见的原因。
2. 影子模式页的 `影子档案` 列表中，目标群组的档案是否处于启用状态。
3. 档案的生效群组是否与实际对话的群号完全一致；私聊场景下为发送者 QQ。
4. 机器人回复是否经由 LLM 生成。影子模式只改写风格，不产生额外回复。
5. 日志等级设为 `debug` 后，回复前应出现 `[LLM Hook] 已准备影子模式档案` 日志；如果出现 `总开关未启用，跳过上下文注入`，即为第 1 条原因。
