...
class ManagerFactory:
    ...
    def __init__(self, config: PluginConfig):
        ...
        # 检查是否启用增强型管理器
        self.use_enhanced = getattr(config, 'use_enhanced_managers', False)
        self.use_sqlalchemy = getattr(config, 'use_sqlalchemy', False)

        logger.info(
            f"[管理器工厂] 初始化完成 "
            f"(SQLAlchemy={self.use_sqlalchemy}, "
            f"增强型管理器={self.use_enhanced})"
        )
    ...
    def create_database_manager(self, context=None):
        ...
        if self.use_sqlalchemy:
            ...
            logger.info("📦 [工厂] 创建 SQLAlchemy 数据库管理器")
            return create_database_manager(self.config, context)
        else:
            from ..services.database_manager import DatabaseManager
            logger.info("📦 [工厂] 创建传统数据库管理器")
            return DatabaseManager(self.config, context)
    ...
    def create_affection_manager(
        ...
        if self.use_enhanced:
            from ..services.enhanced_affection_manager import EnhancedAffectionManager
            logger.info("📦 [工厂] 创建增强型好感度管理器")
            return EnhancedAffectionManager(self.config, database_manager, llm_adapter)
        else:
            from ..services.affection_manager import AffectionManager
            logger.info("📦 [工厂] 创建传统好感度管理器")
    ...
    def create_memory_manager(
        ...
        """
        if self.use_enhanced:
            from ..services.enhanced_memory_graph_manager import EnhancedMemoryGraphManager
            logger.info("📦 [工厂] 创建增强型记忆图管理器")
            ...
            logger.info("📦 [工厂] 创建传统记忆图管理器")
            return MemoryGraphManager.get_instance(
                self.config,
                database_manager,
                llm_adapter,
                decay_manager
    ...
    def create_psychological_manager(
        ...
        """
        if self.use_enhanced:
            from ..services.enhanced_psychological_state_manager import EnhancedPsychologicalStateManager
            logger.info("📦 [工厂] 创建增强型心理状态管理器")
            ...
            from ..services.psychological_state_manager import PsychologicalStateManager
            logger.info("📦 [工厂] 创建传统心理状态管理器")
            return PsychologicalStateManager(
                self.config,
                database_manager,
                llm_adapter,
                affection_manager
    ...
    def create_social_relation_manager(
        ...
        # 所以这里不需要区分
        from ..services.enhanced_social_relation_manager import EnhancedSocialRelationManager
    ...
    def create_time_decay_manager(
        ...
        """创建时间衰减管理器"""
        from ..services.time_decay_manager import TimeDecayManager
    ...
    def get_configuration_info(self) -> dict:
        ...
        return {
            ...
            'use_enhanced_managers': self.use_enhanced,
