...
class LightweightMLAnalyzer:
    ...
    def __init__(self, config: PluginConfig, db_manager: DatabaseManager, 
        ...
        if not SKLEARN_AVAILABLE:
            ...
            # 初始化策略模型
            self.strategy_model: Optional[Any] = None
...
{self.prompts.ML_ANALYZER_REPLAY_MEMORY_PROMPT.format(
    ...
    async def train_strategy_model(self, X: Any, y: Any, model_type: str = "logistic_regression"):
        """
        训练策略模型（逻辑回归或决策树）。
    ...
    def predict_learning_value(self, features: Any) -> float:
        """
        使用训练好的策略模型预测消息的学习价值。
        features: 单个消息的特征向量。
        返回预测的学习价值（0-1之间）。
    ...
    async def analyze_user_behavior_pattern(self, group_id: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            pattern = {
                ...
                'avg_message_length': _mean([len(msg['message']) for msg in messages]),
    ...
    def _analyze_activity_hours(self, messages: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        return {
            ...
            'activity_variance': _variance(list(hour_counts.values()))
        }
    ...
    def _analyze_message_frequency(self, messages: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        return {
            'avg_interval_minutes': _mean(intervals),
    ...
    async def _analyze_interaction_patterns(self, group_id: str, user_id: str, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'avg_relation_strength': _mean([rel['strength'] for rel in user_relations]) if user_relations else 0.0
            }
    ...
    def _analyze_activity_trend(self, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(counts) >= 3:
            trend_slope = _linear_slope(counts)
        else:
