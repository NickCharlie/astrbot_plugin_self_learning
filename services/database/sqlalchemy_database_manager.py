...
class SQLAlchemyDatabaseManager:
    ...
    # ============================================================
    # Phase 1 (EASY): ORM methods replacing legacy delegation
    # These methods have existing ORM models and only need query logic.
    # ============================================================

    async def delete_style_review_by_id(self, review_id: int) -> bool:
        """Delete a style learning review record by ID."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import delete as sa_delete
                from ...models.orm.learning import StyleLearningReview
                stmt = sa_delete(StyleLearningReview).where(StyleLearningReview.id == review_id)
                result = await session.execute(stmt)
                await session.commit()
                if result.rowcount > 0:
                    logger.info(f"[SQLAlchemy] Deleted style review ID: {review_id}")
                    return True
                logger.warning(f"[SQLAlchemy] Style review not found, ID: {review_id}")
                return False
        except Exception as e:
            logger.error(f"[SQLAlchemy] Failed to delete style review: {e}")
            return False

    async def get_persona_learning_review_by_id(self, review_id: int) -> Optional[Dict[str, Any]]:
        """Get a persona learning review record by ID."""
        try:
            async with self.get_session() as session:
                from ...models.orm.learning import PersonaLearningReview
                ...
                return {
                    ...
                    'original_content': review.original_content,
                    'new_content': review.new_content,
                    'proposed_content': review.proposed_content or review.new_content,
                    'confidence_score': review.confidence_score if review.confidence_score is not None else 0.5,
                    'reason': review.reason,
                    'status': review.status,
                    'reviewer_comment': review.reviewer_comment,
                    'review_time': review.review_time,
            ...
            logger.error(f"[SQLAlchemy] Failed to get persona review: {e}")
            return None

    async def update_persona_learning_review_status(
        self, review_id: int, status: str, comment: str = None, modified_content: str = None
    ) -> bool:
        """Update persona learning review status."""
        try:
            async with self.get_session() as session:
                from ...models.orm.learning import PersonaLearningReview
                review = await session.get(PersonaLearningReview, review_id)
                if not review:
                    logger.warning(f"[SQLAlchemy] Persona review not found, ID: {review_id}")
                    return False
                ...
                await session.commit()
                logger.info(f"[SQLAlchemy] Persona review updated, ID: {review_id}, status: {status}")
                return True
        except Exception as e:
            logger.error(f"[SQLAlchemy] Failed to update persona review status: {e}")
            return False

    async def get_recent_bot_responses(self, group_id: str, limit: int = 10) -> List[str]:
        """Get recent bot responses for a group (for diversity analysis)."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select
                from ...models.orm.message import BotMessage
                stmt = (
                    select(BotMessage.message)
            ...
            logger.error(f"[SQLAlchemy] Failed to get recent bot responses: {e}")
            return []

    async def get_recent_week_expression_patterns(
        self, group_id: str = None, limit: int = 20, hours: int = 168
    ) -> List[Dict[str, Any]]:
        """Get expression patterns from the last N hours."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select
                ...
                stmt = (
                    ...
                    .where(ExpressionPattern.last_active_time > time_threshold)
                )
                if group_id is not None:
            ...
            logger.error(f"[SQLAlchemy] Failed to get expression patterns: {e}")
            return []

    # ============================================================
    # Phase 2 (MEDIUM): ORM methods with minor model extensions
    # ============================================================
    ...
    async def add_filtered_message(self, filtered_data: Dict[str, Any]) -> int:
        ...
        try:
            async with self.get_session() as session:
                ...
                quality_scores = filtered_data.get('quality_scores', {})
                if isinstance(quality_scores, dict):
                    quality_scores = json.dumps(quality_scores, ensure_ascii=False)

                msg = FilteredMessage(
                    raw_message_id=filtered_data.get('raw_message_id'),
                    message=filtered_data.get('message', ''),
                    sender_id=filtered_data.get('sender_id', ''),
                    group_id=filtered_data.get('group_id', ''),
                    timestamp=filtered_data.get('timestamp') or current_time,
                    confidence=filtered_data.get('confidence', 0.8),
            ...
            logger.error(f"[SQLAlchemy] Failed to add filtered message: {e}")
            return 0
    ...
    async def get_messages_by_group_and_timerange(
        ...
        """Get raw messages for a group within a time range."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select
                from ...models.orm.message import RawMessage
                stmt = select(RawMessage).where(RawMessage.group_id == group_id)

                if start_time is not None:
                    stmt = stmt.where(RawMessage.timestamp >= start_time)
                if end_time is not None:
                    stmt = stmt.where(RawMessage.timestamp <= end_time)

                stmt = stmt.order_by(RawMessage.timestamp.desc()).limit(limit)
                result = await session.execute(stmt)
                return [
                    {
                        'id': m.id,
            ...
            logger.error(f"[SQLAlchemy] Failed to get messages by timerange: {e}")
            return []
    ...
    async def save_bot_message(
        ...
        try:
            ...
            logger.error(f"[SQLAlchemy] Failed to save bot message: {e}")
            return False

    async def get_recent_learning_sessions(self, days: int = 7) -> List[Dict[str, Any]]:
        """Get recent learning sessions within the specified number of days."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select
                ...
                stmt = (
                    ...
                    .where(LearningSession.start_time > time_threshold)
            ...
            logger.error(f"[SQLAlchemy] Failed to get recent learning sessions: {e}")
            return []

    # ============================================================
    # Phase 3 (HARD): ORM methods requiring new ORM models
    # ============================================================

    async def load_user_profile(self, qq_id: str) -> Optional[Dict[str, Any]]:
        """Load a user profile by QQ ID."""
        try:
            async with self.get_session() as session:
            ...
            logger.error(f"[SQLAlchemy] Failed to load user profile: {e}")
            return None
    ...
    async def save_user_profile(self, qq_id: str, profile_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"[SQLAlchemy] Failed to save user profile: {e}")
            return False

    async def load_user_preferences(self, user_id: str, group_id: str) -> Optional[Dict[str, Any]]:
        """Load user preferences for a specific group."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select, and_
            ...
            logger.error(f"[SQLAlchemy] Failed to load user preferences: {e}")
            return None

    async def save_user_preferences(self, user_id: str, group_id: str, prefs: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"[SQLAlchemy] Failed to save user preferences: {e}")
            return False

    async def load_emotion_profile(self, user_id: str, group_id: str) -> Optional[Dict[str, Any]]:
        """Load emotion profile for a user in a group."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select, and_
            ...
            logger.error(f"[SQLAlchemy] Failed to load emotion profile: {e}")
            return None

    async def save_emotion_profile(self, user_id: str, group_id: str, profile: Dict[str, Any]) -> bool:
        """Upsert emotion profile."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select, and_
                from ...models.orm.psychological import EmotionProfile
            ...
            logger.error(f"[SQLAlchemy] Failed to save emotion profile: {e}")
            return False
    ...
    async def load_style_profile(self, profile_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"[SQLAlchemy] Failed to load style profile: {e}")
            return None

    async def save_style_profile(self, profile_name: str, profile_data: Dict[str, Any]) -> bool:
        """Upsert a style profile."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select
                from ...models.orm.expression import StyleProfile
            ...
            logger.error(f"[SQLAlchemy] Failed to save style profile: {e}")
            return False

    async def save_style_learning_record(self, record_data: Dict[str, Any]) -> bool:
        """Save a style learning record."""
        try:
            async with self.get_session() as session:
            ...
            logger.error(f"[SQLAlchemy] Failed to save style learning record: {e}")
            return False

    async def save_language_style_pattern(
        self, language_style: str, pattern_data: Dict[str, Any]
    ) -> bool:
        """Upsert a language style pattern."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select
            ...
            logger.error(f"[SQLAlchemy] Failed to save language style pattern: {e}")
            return False

    async def get_current_bot_mood(self, group_id: str) -> Optional[Dict[str, Any]]:
        """Get the currently active bot mood for a group."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select, and_
                ...
                stmt = (
                    select(BotMood)
                    .where(and_(BotMood.group_id == group_id, BotMood.is_active == 1))
                    .order_by(BotMood.start_time.desc())
                    .limit(1)
                )
                result = await session.execute(stmt)
            ...
            logger.error(f"[SQLAlchemy] Failed to get current bot mood: {e}")
            return None

    async def backup_persona(self, backup_data: Dict[str, Any]) -> bool:
        """Save a persona backup."""
        try:
            async with self.get_session() as session:
            ...
            logger.error(f"[SQLAlchemy] Failed to backup persona: {e}")
            return False

    async def get_persona_backups(self, limit: int = 10) -> List[Dict[str, Any]]:
        """Get recent persona backups."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select
                from ...models.orm.psychological import PersonaBackup
                stmt = (
                    ...
                    .order_by(PersonaBackup.timestamp.desc())
                    .limit(limit)
                )
                result = await session.execute(stmt)
                return [
            ...
            logger.error(f"[SQLAlchemy] Failed to get persona backups: {e}")
            return []

    async def clear_all_messages_data(self) -> bool:
        """Clear all message and learning data (bulk delete across tables)."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import delete as sa_delete
                ...
                from ...models.orm.reinforcement import (
                    ReinforcementLearningResult, PersonaFusionHistory, StrategyOptimizationResult
                )
                from ...models.orm.performance import LearningPerformanceHistory
                ...
                for table in tables:
                    try:
                        ...
                        logger.warning(f"[SQLAlchemy] Failed to clear {table.__tablename__}: {table_err}")

                await session.commit()
                logger.info("[SQLAlchemy] All message and learning data cleared")
                return True
        except Exception as e:
            logger.error(f"[SQLAlchemy] Failed to clear all messages data: {e}")
            return False

    async def export_messages_learning_data(self, group_id: str = None) -> Dict[str, Any]:
        """Export raw and filtered messages for learning."""
        try:
            async with self.get_session() as session:
                from sqlalchemy import select
                from ...models.orm.message import RawMessage, FilteredMessage
                ...
                raw_result = await session.execute(raw_stmt.order_by(RawMessage.timestamp.desc()).limit(1000))
                filtered_result = await session.execute(filtered_stmt.order_by(FilteredMessage.timestamp.desc()).limit(1000))

                raw_messages = [
                    {
                        'id': m.id, 'sender_id': m.sender_id, 'sender_name': m.sender_name,
                        'message': m.message, 'group_id': m.group_id, 'timestamp': m.timestamp,
                    }
                    for m in raw_result.scalars().all()
                ]
                filtered_messages = [
                    {
                        'id': m.id, 'message': m.message, 'sender_id': m.sender_id,
                        'group_id': m.group_id, 'confidence': m.confidence,
                        'quality_scores': json.loads(m.quality_scores) if m.quality_scores else {},
                        'timestamp': m.timestamp,
                    }
                    for m in filtered_result.scalars().all()
                ]
                return {
            ...
            logger.error(f"[SQLAlchemy] Failed to export messages: {e}")
            return {'raw_messages': [], 'filtered_messages': [], 'raw_count': 0, 'filtered_count': 0}
