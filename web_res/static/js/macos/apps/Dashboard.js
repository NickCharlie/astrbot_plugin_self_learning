...
window.AppDashboard = {
  ...
  data() {
    return {
      loading: true,
      metrics: {},
      trends: { message_growth: 0, filtered_growth: 0, llm_growth: 0, sessions_growth: 0 },
      analyticsData: {},
      styleData: {},
      pieRange: '24h',
      trendRange: '24h',
  ...
  computed: {
    totalLLMCalls() {
      ...
      return Object.values(calls).reduce(function (sum, m) { return sum + (m.total_calls || 0); }, 0);
    },
  },
  ...
  methods: {
    ...
    async fetchAllData() {
      try {
        var results = await Promise.allSettled([
          ...
          fetch('/api/metrics/trends').then(function (r) { return r.json(); }),
          fetch('/api/analytics/trends').then(function (r) { return r.json(); }),
          fetch('/api/style_learning/results').then(function (r) { return r.json(); }),
        ]);

        if (results[0].status === 'fulfilled') this.metrics = results[0].value || {};
        ...
        if (results[2].status === 'fulfilled') this.analyticsData = results[2].value || {};
        if (results[3].status === 'fulfilled') this.styleData = results[3].value || {};
      } catch (e) {
    ...
    /* ---------- 1. LLM 模型调用分布 - 饼图 ---------- */
    updateLLMPie() {
      var chart = this.chartInstances['llmPieChart'] || this.initChart('llmPieChart');
      if (!chart) return;
    ...
    /* ---------- 2. 消息处理趋势 - 折线图 ---------- */
    updateMessageTrend() {
      var chart = this.chartInstances['messageTrendChart'] || this.initChart('messageTrendChart');
      if (!chart) return;
      ...
      if (hourlyData.length > 0) {
        hours = hourlyData.map(function (i) { return i.time; });
        rawMessages = hourlyData.map(function (i) { return i.raw_messages; });
        filteredMessages = hourlyData.map(function (i) { return i.filtered_messages; });
      } else {
        // 生成空的 24 小时时间轴
        hours = [];
        for (var i = 23; i >= 0; i--) {
      ...
      chart.setOption({
        ...
        series: [
          {
            name: '原始消息', type: 'line', data: rawMessages, smooth: true,
            itemStyle: { color: '#2196f3' }, areaStyle: { opacity: 0.3 },
          },
          {
            name: '筛选消息', type: 'line', data: filteredMessages, smooth: true,
            itemStyle: { color: '#4caf50' }, areaStyle: { opacity: 0.3 },
          },
        ],
      }, true);
    },
    ...
    updateResponseTime() {
      var echarts = window.echarts;
      var chart = this.chartInstances['responseTimeChart'] || this.initChart('responseTimeChart');
      if (!chart) return;

      var llmData = this.metrics.llm_calls || {};
      var models = Object.keys(llmData);
      var times = Object.values(llmData).map(function (s) { return s.avg_response_time_ms || 0; });
      ...
      chart.setOption({
        ...
        series: [{
          name: '平均响应时间', type: 'bar', data: times,
    ...
    /* ---------- 4. 学习进度概览 - 仪表盘 ---------- */
    updateLearningGauge() {
      var chart = this.chartInstances['learningGaugeChart'] || this.initChart('learningGaugeChart');
      if (!chart) return;
    ...
    /* ---------- 5. 系统状态监控 - 雷达图 ---------- */
    updateSystemRadar() {
      var chart = this.chartInstances['systemRadarChart'] || this.initChart('systemRadarChart');
      if (!chart) return;
      ...
      // 消息抓取效率
      var messageCapture = totalMessages > 0 ? Math.min(100, (totalMessages / 1000) * 100) : 0;
      // 数据筛选质量
      var filteringQuality = totalMessages > 0 ? (filteredMessages / totalMessages) * 100 : 0;
      // LLM 调用健康度
      var llmModels = Object.values(stats.llm_calls || {});
      var avgSuccessRate = llmModels.length > 0
        ? (llmModels.reduce(function (s, m) { return s + (m.success_rate || 0); }, 0) / llmModels.length) * 100
        : 0;
      // 学习质量
      var learningQuality = stats.learning_efficiency || 0;
      // 响应速度
      var avgResponseTime = llmModels.length > 0
        ? llmModels.reduce(function (s, m) { return s + (m.avg_response_time_ms || 0); }, 0) / llmModels.length
        : 2000;
      var responseSpeed = Math.max(0, 100 - avgResponseTime / 20);
      // 系统稳定性
      var sm = stats.system_metrics || {};
      var systemStability = (Math.max(0, 100 - (sm.cpu_percent || 0)) + Math.max(0, 100 - (sm.memory_percent || 0))) / 2;
    ...
    updateStyleChart() {
      var echarts = window.echarts;
      var chart = this.chartInstances['styleChart'] || this.initChart('styleChart');
      if (!chart) return;

      var data = this.styleData;
      if (data.error || !data.style_progress || data.style_progress.length === 0) {
        chart.setOption(this.emptyOption(data.error || '暂无风格学习数据'), true);
        return;
      }
      ...
      var confidenceData = styleProgress.map(function (item) { return (item.quality_score || 0) * 100; });
      var sampleData = styleProgress.map(function (item) { return item.filtered_count || item.message_count || 0; });
      ...
      chart.setOption({
        ...
        series: [
          {
            name: '平均置信度(%)', type: 'bar', data: confidenceData,
            itemStyle: { color: barColor },
          },
          {
            name: '样本数量', type: 'line', yAxisIndex: 1, data: sampleData,
            itemStyle: { color: '#f093fb' }, lineStyle: { width: 3 },
          },
        ],
      }, true);
    },
    ...
    /* ---------- 7. 用户活跃度热力图 ---------- */
    updateHeatmap() {
      var chart = this.chartInstances['heatmapChart'] || this.initChart('heatmapChart');
      if (!chart) return;

      var heatmapRaw = this.analyticsData.activity_heatmap || {};
      var actualData = heatmapRaw.data || [];
      var days = heatmapRaw.days || ['周一','周二','周三','周四','周五','周六','周日'];
      ...
      chart.setOption({
        ...
        series: [{
          name: '活跃度', type: 'heatmap', data: actualData,
          label: { show: false },
          emphasis: {
