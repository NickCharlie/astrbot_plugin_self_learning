...
class PersonaWebManager:
        ...
        if self.persona_manager:
            try:
                ...
                if not hasattr(self.persona_manager, 'personas'):
                    logger.debug("PersonaManager缺少personas属性，需要初始化")
                    should_initialize = True
                elif not self.persona_manager.personas:
                    logger.debug("PersonaManager的personas列表为空，需要初始化")
                    should_initialize = True
                elif len(self.persona_manager.personas) == 0:
                    logger.debug("PersonaManager的personas列表长度为0，需要初始化")
                    should_initialize = True
                else:
    ...
    async def get_all_personas_for_web(self) -> List[Dict[str, Any]]:
        ...
        try:
            logger.debug("get_all_personas_for_web被调用")
            ...
            if not self._personas_cache:
                ...
                logger.debug("缓存已过期，需要刷新")
                needs_refresh = True
            
            if needs_refresh:
                await self.refresh_personas_cache()
            
            logger.debug(f"当前缓存中有 {len(self._personas_cache)} 个人格")
            
            persona_list = []
            for i, persona in enumerate(self._personas_cache):
                try:
                    logger.debug(f"处理人格 {i}: {getattr(persona, 'persona_id', 'unknown')}")
            ...
            logger.debug(f"返回 {len(persona_list)} 个人格给Web界面")
            return persona_list
